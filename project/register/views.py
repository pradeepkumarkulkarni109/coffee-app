    ...
    try:
        if request.method == 'POST':
            ...
            try:
                ...
                session_otp = requests.post('https://2factor.in/API/V1/e3dde474-6266-11e9-90e4-0200cd936042/SMS/+91'+phone_number+'/AUTOGEN', params=request.POST)
                ...
                return Response({'status':'success', 'statuscode':'400', 'message': 'otp sent to ur mobile number!', 'token':token})
...
def validate_registration_otp(request):
    try:
        if request.method == 'POST':
            ...
            verified = requests.post('https://2factor.in/API/V1/e3dde474-6266-11e9-90e4-0200cd936042/SMS/VERIFY/'+x+'/'+str(get_otp), params=request.POST)
...
def login_view(request):
    ...
    try:
        ...
        if user is not None:
            x = Signup.objects.get(email=email)
            x.is_active = True
            x.save()
            payload = {
                'id': x.id
        ...
        return Response({'status':'Error', 'statuscode':'400', 'message': str(e)})#'Invalid emailId and password! Pleas Enter valid EmailId and Password'})
